...
class Copy {
  ...
  static String getActivity(HttpActivity data) {
    ...
    activityDetails.writeln('\n--- Request ---');
    activityDetails.writeln(
        'Headers: ${Helper.encodeRawJson(data.request?.headers).isJson ? Helper.encodeRawJson(data.request?.headers).prettify : Helper.encodeRawJson(data.request?.headers)}');

    activityDetails.writeln(
        'Query Parameters: ${Helper.encodeRawJson(data.request?.queryParameters).isJson ? Helper.encodeRawJson(data.request?.queryParameters).prettify : Helper.encodeRawJson(data.request?.queryParameters)}');

    activityDetails.writeln(
        'Body: ${(data.request?.body ?? '').isJson ? data.request?.body.prettify : data.request?.body}');
    ...
    activityDetails.writeln(
        'Body: ${(data.response?.body ?? '').isJson ? data.response?.body.prettify : isImage ? "Image Body" : data.response?.body}');
